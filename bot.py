...
def update_product_field(code, field, value):
    if field not in {"name", "price", "photo", "category", "active"}:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_member":
        if not await is_member(context.bot, user_id):
            ...
            return
        await query.message.reply_text("✅ عضویت تأیید شد.", reply_markup=main_keyboard(user_id))
    ...
    if data.startswith("admin_delete:"):
        ...
        await query.message.reply_text(f"⚠️ محصول {code} حذف شود؟", reply_markup=keyboard)
        return
...
def create_zibal_payment(order):
    if not ZIBAL_MERCHANT:
        raise RuntimeError("ZIBAL_MERCHANT تنظیم نشده است")
    if not PUBLIC_URL:
        raise RuntimeError("PUBLIC_URL تنظیم نشده است")
    ...
    print("ZIBAL REQUEST:", response.status_code, response.text[:1200], flush=True)
...
def verify_zibal(track_id):
    ...
    print("ZIBAL VERIFY:", response.status_code, response.text[:1200], flush=True)
    response.raise_for_status()
...
class PaymentHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            stock_ok, remaining = finalize_paid(order["id"], ref_number)
            ...
            if ADMIN_CHAT_ID:
                ...
                telegram_send(ADMIN_CHAT_ID, admin_text)
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ud.get("admin_price_code"):
        ...
        update_product_field(code, "price", int(price_text))
        context.user_data.clear()
    ...
    if ud.get("ordering"):
        ...
        if step == "address":
            ...
            try:
                ...
                payment_url = f"{ZIBAL_START_URL}{track_id}"
                total = int(product["price"]) + SHIPPING_COST
                ...
                error_text = repr(e)
                print("PAYMENT ERROR:", error_text, flush=True)
                context.user_data.clear()
    ...
    if text == "🔥 جدیدترین مدل‌ها":
        ...
        await show_category(update.message, "women")

    elif text == "👞 کفش مردانه":
        await show_category(update.message, "men")
...
async def media_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ud.get("admin_photo_code"):
        ...
        update_product_field(code, "photo", file_id)
        context.user_data.clear()
